...
sub import {
  ...
  foreach my $i (@interfaces) {
    ...
    do {
      ...
      die "$callerpackage: interface $i does not exist" if($@);
    };
...
sub check_implements {
  ...
  for $i (grep { defined &{$implements.'::'.$_} } keys %{$implements.'::'}) {
...
The flip side of polymorphism is type safety. In Perl, ->isa() lets
...
of inheritance (is-a) accrost the whole
...
See http://www.slowass.net/wiki/ for more on Perl OO, including information
...
When implementing an interface, interface.pm searches only that case - not
the classes listed in @ISA. This leads to incomplete interface requirements
when using a subclass as the interface.
